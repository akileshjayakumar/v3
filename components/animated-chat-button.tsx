...
export default function AnimatedChatButton({
  ...
  return (
    ...
    >
      ...
      <span className="absolute inset-0 rounded-md bg-sky-400/30 animate-pulse hidden sm:block" />

      {/* Subtle glow effect on hover */}
      <span className="absolute inset-0 rounded-md bg-sky-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

      {/* Animated border on hover */}
      <span className="absolute inset-0 rounded-md border-2 border-transparent group-hover:border-sky-400/40 transition-colors duration-300" />
