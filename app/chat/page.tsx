      ...
      <div className="flex-1 overflow-hidden flex flex-col">
              ...
              >
                ...
                >
                  {m.role === "user" ? (
                    ...
                    <Star className="w-3 h-3 sm:w-5 sm:h-5 text-gray-700 fill-gray-700" />
                  )}
                </div>
                ...
                >
                  ...
                  {m.role === "user" && (
                    <div className="text-xs text-gray-500 mb-1 mr-2 flex justify-end">
                      You
                    </div>
                  )}
                  <div
                    className={cn(
                      "rounded-3xl px-4 sm:px-5 py-3 sm:py-3.5 transition-all break-words overflow-wrap-anywhere word-break",
                      m.role === "user"
                          ...
                          components={{
                            ...
                            li: (props) => (
                              <li
                                {...props}
                                className={cn(
                                  "leading-relaxed marker:font-medium break-words",
                                  m.role === "user"
                            ...
                            ul: (props) => (
                              <ul
                                {...props}
                                className="space-y-1.5 my-3 list-disc list-outside ml-4 break-words"
                              />
                            ),
                            ol: (props) => (
                              <ol
                                {...props}
                                className="space-y-1.5 my-3 list-decimal list-outside ml-4 break-words"
                              />
                            ),
                            h1: (props) => (
                              <h1
                            ...
                            code: (props) => (
                              <code
                                {...props}
                                className="bg-gray-100 px-1 py-0.5 rounded text-xs sm:text-sm font-mono break-words"
                              />
                            ),
                            pre: (props) => (
                              <pre
                                {...props}
                                className="bg-gray-50 border border-gray-200 rounded-lg p-3 sm:p-4 overflow-x-auto my-3 break-words whitespace-pre-wrap"
                              />
                            ),
                          }}
                        >
        ...
        <div className="shrink-0 border-t border-gray-200 bg-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
            ...
            >
              <div className="flex-1 relative rounded-full bg-white border-2 border-black shadow-sm hover:border-gray-700 transition-colors duration-200">
                <Textarea
                  value={input}
                  onChange={(e) => setInput(e.target.value)}
                  onKeyDown={onComposerKeyDown}
                  placeholder="Message Akilesh..."
                  rows={1}
                  className="w-full min-h-[52px] sm:min-h-[56px] max-h-32 resize-none rounded-full border-0
                           bg-transparent px-5 sm:px-6 py-3 pr-14 sm:pr-16 text-gray-900 placeholder:text-gray-400
                           focus:outline-none focus:ring-0 transition-all duration-200 text-sm sm:text-base"
                />
                <Button
                  type="submit"
                  disabled={isSubmitting || !input.trim()}
                  className={cn(
                    "absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 rounded-full p-0",
                    "bg-black hover:bg-gray-800",
                    "text-white transition-all duration-200",
                    "disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-black",
                    "hover:shadow-lg active:scale-95 flex items-center justify-center"
                  )}
                >
